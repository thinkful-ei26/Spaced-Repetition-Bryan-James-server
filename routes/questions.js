...
questionRouter.post('/data', (req, res, next) => {
    ...
    return User.findOne({ _id: userId }).then(currentUser => {
        ...
        if (answer === theRight.toLowerCase()) {
            ...
            captureNewSpotQuestion.next =
                currentUser.questions[whichQuestion].id
            for (let i = 0; i < qArray.length; i++) {
                if (
                    qArray[i].next === qArray[whichQuestion].id &&
                    !Object.is(qArray[i], captureNewSpotQuestion)
                ) {
            ...
            replyMessage = {
                feedback: `Sorry the correct answer is: ${theRight}`,
            }

            qArray[whichQuestion].m = 1

            let helperIndexValue = qArray[whichQuestion].id
            let captureNewSpot = null
            for (
                let i = qArray[whichQuestion].id;
                i < qArray[whichQuestion].id + 1;
                i++
            ) {
            ...
            let newSpotToMoveTo = qArray[captureNewSpot]

            let helperNextSpot = newSpotToMoveTo.next
            ...
            for (let i = 0; i < qArray.length; i++) {
                if (
                    qArray[i].next === qArray[whichQuestion].id &&
                    !Object.is(qArray[i], newSpotToMoveTo)
                ) {
                    qArray[i].next = OldCurrentNext
                }
            }
        }
        return User.findOneAndUpdate({ _id: userId }, updatePackage, {
            new: true,
            $set: 1,
        })
...
questionRouter.get('/reset', (req, res, next) => {
    ...
    let freshArray = {
        ...
        questions: [
            {
                Question:
                    'A Python data type that holds an ordered collection of values, which can be of any type. This is equivalant to an "array" in many other languages. Python <blank>s are "mutable," implying that they can be changed once created.',
                Answer: 'list',
                m: 1,
                next: 1,
                id: 0,
            ...
            {
                Question:
                    '<blank>s are assigned values using the = operator, which is not to be confused with the == sign used for testing equality. A <blank> can hold almost any type of value such as lists, dictionaries, functions.',
                Answer: 'variable',
                m: 1,
                next: 2,
                id: 1,
            ...
            {
                Question:
                    'Python builds <blank>s using the syntax: def <blank>_name(variable): <blank>s can be stand-alone or can return values. <blank>s can also contain other functions.',
                Answer: 'function',
                m: 1,
                next: 3,
                id: 2,
            ...
            {
                Question:
                    '<blank>s store characters and have many built-in convenience methods that let you modify their content.',
                Answer: 'string',
                m: 1,
                next: 4,
                id: 3,
            ...
            {
                Question:
                    'Using <blank> returns the number of _top-level_ items contained in the object being queried. Length of string',
                Answer: 'len()',
                m: 1,
                next: 5,
                id: 4,
            ...
            {
                Question:
                    'The <blank> function returns a list of integers, the sequence of which is defined by the arguments passed to it. Used in for loops.',
                Answer: 'range',
                m: 1,
                next: 7,
                id: 6,
            ...
            {
                Question:
                    'A <blank> loop permits code to execute repeatedly until a certain condition is met. This is useful if the number of iterations required to complete a task is unknown prior to flow entering the loop.',
                Answer: 'while',
                m: 1,
                next: 8,
                id: 7,
            ...
            {
                Question:
                    'Using the <blank> function allows you to represent the content of a variable as a string, provided that the data type of the variable provides a neat way to do so. <blank> does not change the variable in place, it returns a stringified version of it.',
                Answer: 'str()',
                m: 1,
                next: 9,
                id: 8,
            ...
            {
                Question:
                    'Name given to the class that is being inherited from',
                Answer: 'superclass',
                m: 1,
                next: 0,
                id: 9,
